...
const CONNECTION_ID_EMAIL =
  process.env.NEXT_PUBLIC_KINDE_CONNECTION_ID_EMAIL ?? "";
const CONNECTION_ID_GOOGLE =
  process.env.NEXT_PUBLIC_KINDE_CONNECTION_ID_GOOGLE ?? "";
...
export default function SignInPage() {
  ...
  function handleEmailSubmit(e: React.FormEvent) {
    ...
    if (CONNECTION_ID_EMAIL) params.set("connection_id", CONNECTION_ID_EMAIL);
  ...
  function handleGoogleLogin() {
    ...
    if (CONNECTION_ID_GOOGLE) params.set("connection_id", CONNECTION_ID_GOOGLE);
    window.location.href = `/api/auth/login?${params.toString()}`;
  }
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="relative my-6">
          ...
          <div className="relative flex justify-center text-xs uppercase">
            <span className="bg-zinc-50 px-2 text-zinc-400 dark:bg-black">
              Or continue with
            </span>
          </div>
        </div>
        ...
        <button
          ...
          className="flex h-10 w-full items-center justify-center gap-2 rounded-lg border border-zinc-200 text-sm font-medium transition-colors hover:bg-zinc-100 dark:border-zinc-800 dark:hover:bg-zinc-900"
        >
